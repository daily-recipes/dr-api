package io.codextension.dr.controller;

import io.codextension.dr.model.Recipe;
import io.swagger.annotations.*;
import org.neo4j.driver.Driver;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.HttpStatus;
import org.springframework.http.MediaType;
import org.springframework.web.bind.annotation.*;
import org.springframework.web.client.HttpClientErrorException;

import javax.validation.constraints.NotNull;
import java.util.List;

@Api(tags = "Recipes", description = "Information related to the recipes", produces = MediaType.APPLICATION_JSON_VALUE)
@RestController
@CrossOrigin
public class RecipesController {

    @Autowired
    private Driver driver;

    @ApiOperation(value = "getAllRecipesForMember", notes = "Get all recipes for a member", nickname = "getAllRecipesForMember")
    @ApiResponses(value = {@ApiResponse(code = 500, message = "Server error"),
            @ApiResponse(code = 404, message = "Service not found"),
            @ApiResponse(code = 200, message = "Successful retrieval", response = Recipe.class, responseContainer = "List")})
    @GetMapping(value = "/recipes/chef/{memberId}", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
    public List<Recipe> getAllRecipesForMember(
            @NotNull @PathVariable(required = true) @ApiParam(value = "memberId", required = true) String memberId)
            throws HttpClientErrorException.NotFound {
        throw new HttpClientErrorException(HttpStatus.NOT_FOUND);
    }

    @DeleteMapping(value = "/recipes/{recipeId}/chef/{memberId}", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
    public boolean deleteRecipeForMember(@NotNull @PathVariable(required = true) String memberId,
                                         @NotNull @PathVariable(required = true) Long recipeId) throws HttpClientErrorException.NotFound {
        // delete by id and owner
        throw new HttpClientErrorException(HttpStatus.NOT_FOUND);
    }

    @PutMapping(value = "/recipes/{recipeId}/chef/{memberId}", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
    public Recipe updateRecipeForMember(@NotNull @PathVariable(required = true) String memberId,
                                        @NotNull @PathVariable(required = true) Long recipeId) throws HttpClientErrorException.NotFound {
        throw new HttpClientErrorException(HttpStatus.NOT_FOUND);
    }

    @GetMapping(value = "/recipes/{recipeId}/chef/{memberId}", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
    public Recipe getRecipeForMember(@NotNull @PathVariable(required = true) String memberId,
                                     @NotNull @PathVariable(required = true) Long recipeId) throws HttpClientErrorException.NotFound {
        throw new HttpClientErrorException(HttpStatus.NOT_FOUND);
    }
}